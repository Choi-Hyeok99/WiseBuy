...
> **최근 개선 (2026.09)** — 오래 묵혀둔 프로젝트를 다시 열어 아래를 정리했습니다. 자세한 내용은 [최근 개선 로그](#최근-개선-로그).
> - 보안 정리 (JWT secret / 부하테스트 토큰 하드코딩 제거, Actuator 축소)
> - `docker-compose up` 한 번으로 전체 스택 기동 (누락 서비스·환경변수·Kafka 리스너 정리)
> - **부하테스트 재수행 → 병목 특정 → 개선**: 재고 조회 게이트웨이 경유 p95 4초 → 35ms (요청당 로깅이 병목이었음)
> - 재고 차감 버그 수정 + 동시성 테스트로 오버셀 방지 검증
...
| 경로 | p95 | 비고 |
|---|---|---|
| product 서비스 직접 (`:8082`) | **~7ms** | Redis 캐시 조회. 2,000 RPS까지 여유 |
...
- **Resilience4j**가 import만 돼 있고 실제로 뭔가를 보호하는 Circuit Breaker가 없다. 게이트웨이에 rate limiting(유입 제어)도 없어 폭주를 흡수할 장치가 없다.
- 배치 컨슈머는 at-least-once 재전달에 대비해 멱등키를 뒀지만, DB-Redis 이중 쓰기의 완전한 exactly-once는 아니다(transactional outbox 미적용).
